...
echo dazzle build --repository tjushilei/dazzle-wsfull-build --output-test-xml results.xml -t "$IMAGE_NAME:$BUILD_TAG" -f "$DOCKERFILE" .


/dazzle/dazzle build --repository tjushilei/dazzle-wsfull-build --output-test-xml results.xml -t "$IMAGE_NAME:$BUILD_TAG" -f "$DOCKERFILE" .
